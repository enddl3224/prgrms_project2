...
CFG = "/home/nvidia/xycar_ws/src/yolov3_trt_ros/src/yolov3-tiny_tstl_416_class9.cfg"
TRT = '/home/nvidia/xycar_ws/src/yolov3_trt_ros/src/model_epoch980.trt'
...
class yolov3_trt(object):
    ...
    def detect(self):
        ...
        image_sub = rospy.Subscriber('/usb_cam/image_raw', Imageros, callback, queue_size=1)
        ...
        while not rospy.is_shutdown():
            ...
            if self.show_img:
                ...
                #put text distance to bbox [center, bottom]
                show_img = cv2.cvtColor(obj_detected_img_np, cv2.COLOR_RGB2BGR)
                show_img = self.homography.get_distance(show_img, boxes)
                
                cv2.putText(show_img, "FPS:"+str(int(fps)), (10,50),cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,0),2,1)
                cv2.imshow("result",show_img)
                cv2.waitKey(1)

    def _write_img_message(self, detection_results, boxes, scores, classes):
    ...
    def publisher(self, boxes, confs, classes):
        ...
        self._write_img_message(detection_results, boxes, confs, classes)
...
def callback(image):
    ...
    xycar_image = cv2.remap(img, map1, map2, cv2.INTER_LINEAR)
